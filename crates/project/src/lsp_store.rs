...
impl LocalLspStore {
    ...
    fn find_injection_region_at_offset(
        ...
        for info in layers {
            let region_index = language_counts.entry(info.language_name.clone()).or_insert(0);
    ...
    fn collect_injection_servers_for_region(
        ...
        registrations
            ...
            .filter_map(|(key, reg)| {
                ...
                if let Some(LanguageServerState::Running { server, adapter, .. }) =
    ...
    fn transform_injection_range_to_anchor_range(
        ...
        let start = buffer_snapshot.clip_point_utf16(
            Unclipped(PointUtf16::new(transformed.start.line, transformed.start.character)),
            Bias::Left,
        );
        let end = buffer_snapshot.clip_point_utf16(
            Unclipped(PointUtf16::new(transformed.end.line, transformed.end.character)),
            Bias::Left,
        );
    ...
    fn resolve_injection_context(
        ...
        let transformed_position = Self::map_position_to_injection_region(offset, &snapshot, &region_id)?;

        // Get the virtual URI for this region
        let buffer_id = buffer.read(cx).remote_id();
        let virtual_uri = self.buffer_registrations
            .get(&buffer_id)
            .and_then(|regs| {
                regs.iter()
    ...
    fn register_buffer_with_language_servers(
        ...
        let (reused, lsp_delegate) = self
            ...
            .unwrap_or_else(|| {
                (false, LocalLspAdapterDelegate::from_local_lsp(self, &worktree, cx))
            });
        ...
        for entry in entries {
            ...
            let (content, registration_uri) = match (&injection_region, &entry.injection_language) {
                (Some(region_id), Some(injection_lang)) => {
                    ...
                    let extracted = Self::extract_injection_content_for_region(&buffer_snapshot, region_id);
                    let virtual_uri = Self::create_embedded_content_uri(&uri, region_id, injection_lang);
            ...
            let servers = self
                .lsp_tree
                .walk(
                    ProjectPath { worktree_id, path: path.clone() },
                    language_name.clone(),
            ...
            for server_node in servers {
                ...
                let Some(server_id) = server_node.server_id_or_init(|disposition| {
                    ...
                    let workspace_uri = Uri::from_file_path(worktree.read(cx).absolutize(&server_path.path));
                ...
                let (server, adapter) = match server_state {
                    LanguageServerState::Running { server, adapter, .. } => {
                ...
                self.buffer_registrations
                    .entry(buffer_id)
                    .or_default()
                    .insert(reg_key, BufferRegistration {
...
impl LspStore {
        ...
        if let Some((client, project_id)) = self.upstream_client() {
            ...
            let injection_tasks: Vec<_> = self
                ...
                .map(|local| {
                    ...
                    let mut by_region: HashMap<InjectionRegionId, Vec<(LanguageServerId, Arc<LanguageServer>, lsp::Uri)>> =
                        HashMap::default();
                    ...
                    if let Some(regs) = local.buffer_registrations.get(&buffer_id) {
                        for (key, reg) in regs {
                            if let Some(ref region_id) = key.injection_region {
                                ...
                                if let Some(LanguageServerState::Running { server, .. }) = local.language_servers.get(&key.server_id) {
                                    let entry = by_region.entry(region_id.clone()).or_default();
                                    if !entry.iter().any(|(id, _, _)| *id == key.server_id) {
                                        entry.push((key.server_id, server.clone(), reg.uri.clone()));
                                    }
                                }
                            }
                        }
                ...
                .map(|(region_id, virtual_uri, servers)| {
                    ...
                    async move {
                        ...
                        let mut colors: Vec<(LanguageServerId, collections::HashSet<DocumentColor>)> =
                            Vec::new();

                        // Get injection content for this specific region for position mapping
                        let injection_content = LocalLspStore::extract_injection_content_for_region(&buffer_snapshot, &region_id);
                        ...
                        for (server_id, response) in results {
                            if let Some(color_infos) = response {
                                ...
                                for color_info in color_infos {
                                    ...
                                    let start_offset = LocalLspStore::lsp_position_to_offset(&injection_content, color_info.range.start);
                                    let end_offset = LocalLspStore::lsp_position_to_offset(&injection_content, color_info.range.end);

                                    if let (Some(start_inj), Some(end_inj)) = (start_offset, end_offset) {
                                        let start_parent = LocalLspStore::map_position_from_injection_region(start_inj, &buffer_snapshot, &region_id);
                                        let end_parent = LocalLspStore::map_position_from_injection_region(end_inj, &buffer_snapshot, &region_id);

                                        if let (Some(start_off), Some(end_off)) = (start_parent, end_parent) {
                                            let start_point = buffer_snapshot.offset_to_point_utf16(start_off);
                                            let end_point = buffer_snapshot.offset_to_point_utf16(end_off);

                                            let mapped_range = lsp::Range {
                                                start: lsp::Position { line: start_point.row, character: start_point.column },
                                                end: lsp::Position { line: end_point.row, character: end_point.column },
                                            };
            ...
            cx.background_spawn(async move {
                let main_colors = document_colors_task
                    .await
                    .into_iter()
                    .fold(HashMap::default(), |mut acc, (server_id, colors)| {
                        acc.entry(server_id)
    ...
    pub fn hover(
        ...
        if let Some((client, upstream_project_id)) = self.upstream_client() {
            ...
            let injection_task: Option<Task<Vec<(LanguageServerId, Option<Hover>)>>> =
                if let Some((transformed_pos, ref virtual_uri, ref injection_servers)) = injection_info {
                    ...
                    let col_offset_first_line = position.column as i32 - transformed_pos.character as i32;
                    let buffer_snapshot = buffer.read(cx).snapshot();
                    ...
                    let response_results = injection_servers
                        ...
                        .map(|(server_id, server)| {
                            ...
                            let request = server.request::<lsp::request::HoverRequest>(lsp_params.clone());
                            async move {
                        ...
                        while let Some((server_id, lsp_hover)) = results.next().await {
                            let hover = lsp_hover.and_then(|h| {
                                ...
                                if contents.iter().all(|b| b.text.is_empty()) {
                                    ...
                                    Some(Hover { contents, range: anchor_range, language: None })
                                }
                            });
    ...
    pub fn document_highlights(
        ...
        let buffer_id = buffer.read(cx).remote_id();
        let buffer_snapshot = buffer.read(cx).snapshot();
        let scope = buffer_snapshot.language_scope_at(injection_ctx.original_position.to_offset(&buffer_snapshot));
        ...
        let servers: Vec<_> = injection_servers.into_iter().map(|(id, srv, _)| (id, srv)).collect();

        let lsp_params = lsp::DocumentHighlightParams {
            text_document_position_params: lsp::TextDocumentPositionParams {
                text_document: lsp::TextDocumentIdentifier { uri: injection_ctx.virtual_uri.clone() },
        ...
        let col_offset_first_line = position.column as i32 - injection_ctx.transformed_position.character as i32;

        let response_futures = servers
            .into_iter()
            .map(|(server_id, server)| {
                let request = server.request::<lsp::request::DocumentHighlightRequest>(lsp_params.clone());
        ...
        cx.background_spawn(async move {
            ...
            while let Some((_server_id, lsp_highlights)) = results.next().await {
                let Some(lsp_highlights) = lsp_highlights else { continue };
    ...
    fn sync_injection_registrations_on_edit(
        ...
        for (reg_key, uri) in injection_registrations {
            ...
            let content = LocalLspStore::extract_injection_content_for_region(full_snapshot, region_id);
    ...
    fn on_buffer_reparsed(
        ...
        for region in injection_regions {
            ...
            let virtual_uri = LocalLspStore::create_embedded_content_uri(&uri, &region_id, &region.language);
            ...
            for target_lang_name in languages_to_register {
                ...
                for server_node in servers {
                    ...
                    if let Some(existing) = existing_reg {
                        ...
                        local.buffers_opened_in_servers
                            .entry(buffer_id)
                            .or_default()
                            .insert(server_id);
